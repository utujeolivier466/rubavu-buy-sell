...
function IconZoning() { return <svg viewBox="0 0 24 24" className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M3 3h7v7H3zM14 3h7v7h-7zM14 14h7v7h-7zM3 14h7v7H3z"/></svg>; }
...
function PropertyDetailPage() {
  ...
  async function fetchRelated(current: Property) {
    ...
    const currentCity = normalize((current as any).city);
    ...
    const matches = (data as Property[]).filter((p) => {
      const sameCity = currentCity && normalize((p as any).city) === currentCity;
  ...
  return (
    <>
      <SEOHead
        ...
        description={property.description || `${property.property_type} for ${property.listing_type} in ${property.location_text} — ${property.currency} ${Number(property.price).toLocaleString()}`}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
              ...
              <SpecItem icon={<IconZoning />} label="Zoning" value={property.zoning} />
            </div>
